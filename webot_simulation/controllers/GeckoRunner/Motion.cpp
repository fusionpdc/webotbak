...
//#include"GeckoRunner.hpp"
	void Motion::getNextStep()
	{
		...
		if (state==1)
		{	
			cout<<"satae1  step:"<<t<<endl;
			Detach(RF);moving(RR,0);moving(LF,0);moving(LR,-hd[3]);t++;//Outputall_PWM();
			if(t>step){state=2;setallcurrentposition();t=1;PrintCoordinate();}
		}

		else if (state==2)
		{
			cout<<"satae2 step:"<<t<<endl;
			Adhesive(RF);t++;//Outputall_PWM();
			if(touchState[0]==1){state=3;setallcurrentposition();t=1;PrintCoordinate();hd[0]=origin_pos[0][2]+24;}
		}

		else if (state==3)
		{
			cout<<"satae3 step:"<<t<<endl;
			Detach(RR);moving(RF,-hd[0]);moving(LF,0);moving(LR,0);t++;//Outputall_PWM();
			if(t>step){state=4;setallcurrentposition();t=1;PrintCoordinate();}
		}

		else if (state==4)
		{
			cout<<"satae4 step:"<<t<<endl;
			Adhesive(RR);t++;//outputall_PWM();
			if(touchState[1]==1){state=5;setallcurrentposition();t=1;PrintCoordinate();hd[1]=origin_pos[1][2]+24;}
		}

		else if (state==5)
		{	
			cout<<"satae5 step:"<<t<<endl;
			Detach(LF);moving(RR,-hd[1]);moving(RF,0);moving(LR,0);t++;//Outputall_PWM();
			if(t>step){state=6;setallcurrentposition();t=1;PrintCoordinate();}
		}

		else if (state==6)
		{
			cout<<"satae6 step:"<<t<<endl;
			Adhesive(LF);t++;//Outputall_PWM();
			if(touchState[2]==1){state=7;setallcurrentposition();t=1;PrintCoordinate();hd[2]=origin_pos[2][2]+24;}
		}

		else if (state==7)
		{
			cout<<"satae7 step:"<<t<<endl;
			Detach(LR);moving(RR,0);moving(LF,-hd[2]);moving(RF,0);t++;//Outputall_PWM();
			if(t>step){state=8;setallcurrentposition();t=1;PrintCoordinate();}
		}

		else if (state==8)
		{
			cout<<"satae8 step:"<<t<<endl;
			Adhesive(LR);t++;//Outputall_PWM();
			if(touchState[3]==1){state=1;setallcurrentposition();t=1;PrintCoordinate();hd[3]=origin_pos[3][2]+24;}
		}
...
*/
	...
	void Motion::  moving(int fn,float heightL )
	{
